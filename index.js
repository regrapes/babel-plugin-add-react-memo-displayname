        ...
        for (const declarator of declarations) {
          if (t.isIdentifier(declarator.node.id) && t.isCallExpression(declarator.node.init)) {
            ...
            if (t.isArrowFunctionExpression(declarator.node.init.arguments[0])) {
              ...
              t.isCallExpression(declarator.node.init.arguments[0]) &&
              isAnonymousComponent(t, declarator.node.init.callee) &&
              t.isArrowFunctionExpression(declarator.node.init.arguments[0].arguments[0])
            ) {
              ;[callExpression] = declarator.node.init.arguments
            }

            if (callExpression && isAnonymousComponent(t, callExpression.callee)) {
              path.insertAfter(
                t.variableDeclarator(
                  t.identifier(`${declarator.node.id.name}.displayName`),
